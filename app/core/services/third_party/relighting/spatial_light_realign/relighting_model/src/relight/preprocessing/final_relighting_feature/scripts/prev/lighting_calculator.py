...
def relight_image_pipeline(
    ...
    # ========================================================================
    # STAGE 4: Prepare inputs for Generative Synthesis (IC-Light)
    # ========================================================================
    
    print("\n[STAGE 4] Preparing inputs for Generative Model (IC-Light)...")
    
    # Convert tensors to numpy for IC-Light compatibility
    light_map_np = light_map.cpu().numpy() if isinstance(light_map, torch.Tensor) else light_map
    ...
    if albedo_map is not None:
        if isinstance(albedo_map, torch.Tensor):
            albedo_map_np = albedo_map.cpu().numpy()
        else:
            albedo_map_np = albedo_map
    else:
        albedo_map_np = None
    
    # Prepare Stage 4 inputs
    stage4_inputs = {
        'light_map': light_map_np,  # Guidance map for IC-Light
        'original_image': rgb_image_np,  # Original RGB image
        'albedo': albedo_map_np,  # Base color (optional)
        'light_direction': light_position,  # Light direction info
        'light_color': light_color if light_color is not None else [1.0, 1.0, 1.0]
    }

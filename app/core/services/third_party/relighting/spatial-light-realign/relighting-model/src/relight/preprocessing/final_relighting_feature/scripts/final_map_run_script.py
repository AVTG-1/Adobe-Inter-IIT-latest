...
def process_lighting(normal_path, depth_path, output_dir='./outputs', 
    ...
    import time
    start = time.time()
    light_map = calculator.relight_image_pipeline(
        albedo_map=None,
    ...
    intensity_np = light_map.cpu().numpy()
    
    # Save outputs
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"\n[5] Saving to {output_dir}/")
    np.save(f'{output_dir}/light_map.npy', intensity_np)
    
    # Save as image
    intensity_norm = (intensity_np - intensity_np.min()) / (intensity_np.max() - intensity_np.min() + 1e-8)
    Image.fromarray((intensity_norm * 255).astype(np.uint8)).save(f'{output_dir}/light_map.png')
...
def process_lighting_arrays(normal_map: np.ndarray,
                            depth_map: np.ndarray,
                            output_dir: str = './outputs',
                            light_pos=None,
                            use_gpu: bool = True) -> np.ndarray:
    """
    Process lighting given in-memory normal/depth arrays.
    Saves outputs to output_dir and returns intensity map (H,W).
    """
    # Normalize normals if needed
    if normal_map.min() >= 0:
        if normal_map.max() <= 1.0:
    ...
    if d_max - d_min > 1e-8:
        ...
        depth_map = np.zeros_like(depth_map)  # constant depth

    # Match dims
    normal_map, depth_map = match_dimensions(normal_map.astype(np.float32), depth_map.astype(np.float32))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compute lighting intensity map from normals and depth")
    parser.add_argument("--normal", required=True, help="Path to normals .npy (H,W,3 or 3,H,W)")
    parser.add_argument("--depth", required=True, help="Path to depth .npy (H,W)")
    parser.add_argument("--out", "-o", default="./lightmap_out", help="Output directory")
    ...
    print(f"Saved light map to: {os.path.join(args.out, 'lighting_intensity.png')}")